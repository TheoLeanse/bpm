...
/***/ function(module, exports, __webpack_require__) {
	...
	var submit = document.querySelector('#titleform');
	var remove = document.querySelector('.jsRemove');
	...
	var similarTracks = document.querySelector('#similar');

	var beats = [];

	// handle multiple users
	user.addEventListener('change', function () {
	    _firebase2.default.init(user.value).then(function (data) {
	        return dataDisplay.innerHTML = ObjToUL(data);
	    });
	...
	counters.forEach(function (counter) {
	    return counter.addEventListener('click', function () {
	        var beat = new Date().getTime() / 1000;
	        beats.push(beat);

	        var bpm = BPM(beats);
	        display.innerHTML = bpm ? bpm : '';

	        var similar = range(_firebase2.default.data, bpm - 5, bpm + 5);
	...
	submit.addEventListener('submit', function (e) {
	    e.preventDefault();
	    if (title.value) _firebase2.default.save(user.value, title.value, BPM(beats));
	    reset();
	});

	remove.addEventListener('click', function (e) {
	    if (!edit.checked) return;
	    var trackTitle = /"(.+)"/.exec(e.target.innerHTML)[1];
	    if (confirm('sure you want to delete ' + trackTitle + '?')) {
	        _firebase2.default.remove(user.value, trackTitle);
	        reset();
	    }
	});
	...
	function BPM(beatlist) {
	    var count = beatlist.length - 1;
	    var time = beatlist[count] - beatlist[0];
	...
	function reset() {
	    ...
	    title.value = '';
	    dataDisplay.innerHTML = ObjToUL(_firebase2.default.data);
	}

	function ObjToUL(object) {
	    return JSON.stringify(object).replace(/{/g, '<ul><li>').replace(/,/g, '</li><li>').replace(/}/g, '</li></ul>');
	}

	function range(input, start, end) {
	    return Object.keys(input).filter(function (key) {
	        return start <= input[key] && input[key] <= end;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ascending = __webpack_require__(171);
	...
	exports.default = {
	    init: function init(user) {
	        ...
	        return new Promise(function (resolve) {
	            firebaseDB.ref('/' + user).on('value', function (data) {
	                _this.data = (0, _ascending2.default)(data.val());
	                resolve(_this.data);
	            });
	        });
	    },
	    save: function save(user, title, bpm) {
	        firebaseDB.ref('/' + user + '/' + title).set(bpm);
	    },
	    remove: function remove(user, title) {
...
/***/ function(module, exports) {
	...
	function ascending(input) {
	    return Object.keys(input).sort(function (a, b) {
